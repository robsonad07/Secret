...
app.get("/secrets", async (req, res) => {
  if (req.isAuthenticated()) {
    const result = await db.query("select * from user_secrets where id = $1", [req.user.id]);
    let secrets = [];
...
app.post("/submit", async (req, res) => {
  try { 
    ...
    await db.query("insert into user_secrets (id, secret) values ($1, $2)", [req.user.id, req.body.secret]);
